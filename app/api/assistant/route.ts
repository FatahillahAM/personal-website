...
export async function POST(req: Request) {
  if (!process.env.ANTHROPIC_API_KEY) {
    console.error(
      "[assistant] ANTHROPIC_API_KEY is missing. Add it to .env.local (local) " +
        "or your host's environment variables, then restart/redeploy."
    );
    return Response.json(
      {
        error:
          process.env.NODE_ENV === "production"
            ...
            : "ANTHROPIC_API_KEY is not set. Add it, then restart the dev server. See /api/health.",
      },
      { status: 503 }
    );
  }
